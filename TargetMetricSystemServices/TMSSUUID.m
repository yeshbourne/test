...
+ (NSString *)uniqueID {
    ...
    NSString *DeviceModel = [[TMSSHardwareInfo deviceModel] capitalizedString];
    DeviceModel = [[DeviceModel substringWithRange:NSMakeRange([DeviceModel length] - 1, 1)] capitalizedString];
    
    // Get the System Name
    NSString *SystemName = [[[TMSSHardwareInfo systemName] substringWithRange:NSMakeRange(2, 1)] capitalizedString];
    
    // Get the Device Type
    NSString *DeviceType = [[TMSSHardwareInfo systemDeviceTypeFormatted:NO] capitalizedString];
    DeviceType = [DeviceType substringWithRange:NSMakeRange([DeviceType length] - 1, 1)];
    
    // Get the Screen Width
    NSString *ScreenWidth = [[[NSString stringWithFormat:@"%d", [TMSSHardwareInfo screenWidth]] substringToIndex:1] capitalizedString];
    
    // Get the Screen Height
    NSString *ScreenHeight = [[[NSString stringWithFormat:@"%d", [TMSSHardwareInfo screenHeight]] substringToIndex:1] capitalizedString];
    
    // Check if Multitasking is Enabled
    NSString *MultitaskingEnabled = [[[NSString stringWithFormat:@"%d", [TMSSHardwareInfo multitaskingEnabled]] substringToIndex:1] capitalizedString];
    
    // Check if the proximity sensor exists
    NSString *ProximitySensorEnabled = [[[NSString stringWithFormat:@"%d", [TMSSHardwareInfo proximitySensorEnabled]] substringToIndex:1] capitalizedString];
    
    // Get the number of processors
    NSString *NumberProcessors = [[[NSString stringWithFormat:@"%d", [TMSSProcessorInfo numberProcessors]] substringToIndex:1] capitalizedString];
    
    // Get the processor Speed
    NSString *ProcessorSpeed = [[[NSString stringWithFormat:@"%d", [TMSSProcessorInfo processorSpeed]] substringToIndex:1] capitalizedString];
    
    // Get the Cell MAC Address
    NSString *CellMACAddress = [[[TMSSNetworkInfo cellMACAddress] substringToIndex:1] capitalizedString];
    
    // Get the WiFi MAC Address
    NSString *WiFiMACAddress = [[[TMSSNetworkInfo wiFiMACAddress] substringToIndex:1] capitalizedString];
    
    // Get the WiFi MAC Address different sector (5)
    NSString *WiFiMACAddress2 = [[[TMSSNetworkInfo wiFiMACAddress] substringWithRange:NSMakeRange(4, 1)] capitalizedString];
    
    // Get the WiFi MAC Address different sector (10)
    NSString *WiFiMACAddress3 = [[[TMSSNetworkInfo wiFiMACAddress] substringWithRange:NSMakeRange(9, 1)] capitalizedString];
    
    // Get the total disk space
    NSString *TotalDiskSpace = [[[TMSSDiskInfo diskSpace] substringToIndex:1] capitalizedString];
...
+ (NSString *)deviceSignature {
    ...
    NSString *SystemVersion = [NSString stringWithFormat:@"%d", [[TMSSHardwareInfo systemVersion] intValue]];
    
    // Get the Screen Height
    NSString *ScreenHeight = [NSString stringWithFormat:@"%d", [TMSSHardwareInfo screenHeight]];
    
    // Get the Screen Width
    NSString *ScreenWidth = [NSString stringWithFormat:@"%d", [TMSSHardwareInfo screenWidth]];
    
    // Plugged In
    NSString *PluggedIn = [NSString stringWithFormat:@"%d", [TMSSHardwareInfo pluggedIn]];
    
    // Jailbroken
    NSString *Jailbroken = [NSString stringWithFormat:@"%d", [TMSSJailbreakcheck jailbroken]];
    
    // Headphones Attached
    NSString *HeadphonesAttached = [NSString stringWithFormat:@"%d", [TMSSAccessoryInfo headphonesAttached]];
    
    // Battery Level
    NSString *BatteryLevel = [NSString stringWithFormat:@"%d", (int)[TMSSBatteryInfo batteryLevel]];
    
    // Fully Charged
    NSString *FullyCharged = [NSString stringWithFormat:@"%d", [TMSSBatteryInfo fullyCharged]];
    
    // Connected to WiFi
    NSString *ConnectedtoWiFi = [NSString stringWithFormat:@"%d", [TMSSNetworkInfo connectedToWiFi]];
    
    // Device Orientation
    NSString *DeviceOrientation = [NSString stringWithFormat:@"%d", [TMSSAccelerometerInfo deviceOrientation]];
    
    // Country
    NSString *Country = [[[TMSSLocalizationInfo country] substringToIndex:2] uppercaseString];
    
    // TimeZone
    NSString *TimeZone = [[[TMSSLocalizationInfo timeZone] substringToIndex:2] uppercaseString];
    
    // Number Processors
    NSString *NumberProcessors = [NSString stringWithFormat:@"%d", [TMSSProcessorInfo numberProcessors]];
    
    // Processor Speed
    NSString *ProcessorSpeed = [NSString stringWithFormat:@"%d", [TMSSProcessorInfo processorSpeed]];
    
    // Total Disk Space
    NSString *TotalDiskSpace = [NSString stringWithFormat:@"%d", [[TMSSDiskInfo diskSpace] intValue] ];
    
    // Total Memory
    NSString *TotalMemory = [NSString stringWithFormat:@"%d", (int)[TMSSMemoryInfo totalMemory]];
